...
def validate_epoch(model, val_loader, criterion, device, args, trial=None, epoch=None):
    ...
    for game_names, body_prop, vel, ang_vel, body_nums in val_loader:
...
def run_one_trial(trial, args, device):
    ...
    for epoch in range(args.epochs):
        ...
        for game_names, body_prop, vel, ang_vel, body_nums in train_loader:
            body_prop = body_prop.to(device)
            vel = vel.to(device)
            ang_vel = ang_vel.to(device)
...
def parse_args():
    ...
    p.add_argument("--data_path", type=str, default="/mnt/nfs_project_a/chang/small_data/data/blocktower")
    p.add_argument("--save_dir", type=str, default="exps/optuna_full_search_1scene")
    p.add_argument("--model_name", type=str, default="delete0dim_neural_simulator")
    ...
    p.add_argument("--report_interval", type=int, default=10)
    p.add_argument("--save_trial_ckpt", action="store_true", default=False)

    # 这些会作为默认值（当你不调这些参数时可用）
    ...
    p.add_argument("--dist_boundary", type=float, default=0.0)
    ...
    # Optuna
    p.add_argument("--study_name", type=str, default="joint_search_full_arch_solver")
    p.add_argument("--storage", type=str, default="")
    p.add_argument("--n_startup_trials", type=int, default=12)
    p.add_argument("--n_warmup_steps", type=int, default=80)
...
def main():
    ...
    with open(os.path.join(args.save_dir, "best_params.json"), "w", encoding="utf-8") as f:
        json.dump(best_json, f, ensure_ascii=False, indent=2)

    # 保存所有 trial
    save_trials(study, os.path.join(args.save_dir, "optuna_trials.csv"))

    print(f"Saved: {os.path.join(args.save_dir, 'best_params.json')}")
    print(f"Saved: {os.path.join(args.save_dir, 'optuna_trials.csv')}")
