...
def get_args():
    ...
    parser.add_argument('--dist_boundary', type=float, default=0.03)
...
def run_trial_set(model, data_loader, device):
    """
...
def main():
    ...
    if not os.path.exists(args.save_dir):
        print(f"Error: Save directory {args.save_dir} does not exist.")
        return
    ...
    # 2. 加载训练好的权重
    weights_path = os.path.join(args.save_dir, 'model_best.pt')
    ...
    # 3. 加载数据集
    print(f"Loading Dataset from {args.trial_data_path} ({args.scene_type})...")
    ...
    # 4. 运行推理
    trial_predictions = run_trial_set(model, trial_loader, device)
    
    # 5. 保存结果
    output_path = os.path.join(args.save_dir, 'trial_predictions.npy')
    np.save(output_path, trial_predictions)
    print(f"Done! Saved {len(trial_predictions)} trial predictions to {output_path}")
