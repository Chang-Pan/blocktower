...
class ForceFieldPredictor(nn.Module):
    ...
    def forward(self, init_x,query_x,init_v,query_v,init_angular_v,query_angular_v, scene_scale=None):
        ...
        # 统一变换到施力者init的局部坐标系
        init_quat = init_x_exp[..., 3:7].clone()

        # 1. 相对位置到局部系
        relative_x_world = (query_x_exp[...,:3] - init_x_exp[...,:3]).clone() # [batch, obj_num, target_obj_num, 3]
        ...
        q_rel = self.quat_multiply(init_quat_conj, query_x_exp[..., 3:7]) # [batch, obj_num, target_obj_num, 4]
        query_x_exp = torch.cat([
            relative_x_local,
            q_rel,
            query_x_exp[..., 7:],
        ], dim=-1)
        ...
        identity_quat = torch.cat([
            torch.zeros_like(init_x_exp[..., 3:6]),
            torch.ones_like(init_x_exp[..., 6:7])
        ], dim=-1)
        init_x_exp = torch.cat([
            torch.zeros_like(init_x_exp[..., :3]),
            identity_quat,
            init_x_exp[..., 7:],
        ], dim=-1)
        ...
        query_v_exp = self.quat_rotate_inv(init_quat, rel_v_world)
        ...
        if self.use_dist_mask:
            ...
            transition = boundary * 0.5  # transition width = half of boundary
        ...
        # predict force
        branch_input =  torch.cat([init_x_exp[...,7:10]], dim=-1)  # [batch, obj_num, target_obj_num, 3]
        if self.use_dist_input:
        ...
        # 将局部系力转换回世界系
        init_quat_world = init_x.unsqueeze(2).expand(-1, -1, target_obj_num, -1)[..., 3:7] # [batch, obj_num, target_obj_num, 4]

        # 线性力部分 (前3维) 旋转回世界系
        force_linear_world = self.quat_rotate(init_quat_world, force[...,:3]) # [batch, obj_num, target_obj_num, 3]
        # 力矩部分 (后3维) 也旋转回世界系
        force_torque_world = self.quat_rotate(init_quat_world, force[...,3:6]) # [batch, obj_num, target_obj_num, 3]
        
        force = torch.cat([force_linear_world, force_torque_world], dim=-1) # [batch, obj_num, target_obj_num, 6]
        ...
        ground_transition = ground_threshold * 0.3  # 过渡宽度
